...
from .models import IllustrationType, UsePermission, Podcast
...
class PodcastForm(ItemForm):
    title = forms.CharField(**dchar_required)
    speakers= forms.CharField(**dchar)
    audio_link= forms.CharField(**dchar)
    date = forms.CharField(**dchar)
    description =forms.CharField(**dtext)

    class Meta:
        model = Podcast
        m = 'title,speakers,audio_link,date,description,incomplete,complete'
        m += ',approved'
        fields = m.split(',')
